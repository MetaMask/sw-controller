  ...
  registerWorker () {
    ...
    .then((registerdWorker) => {
      let timeOutId = setTimeout(() => {
        if (this.serviceWorkerApi.controller) return Promise.resolve(this.serviceWorkerApi.controller)
        if (registerdWorker.active) return Promise.resolve(registerdWorker.active)
        return Promise.reject(new Error('ClientSideServiceWorker: No controller found and onupdatefound timed out'))
